...
class PaginationHelper
{
    ...
    {
        $paginator = new Paginator($query); // Aus Performance den OutputWalker nicht nutzen ;)
        $paginator->setUseOutputWalkers(false); // Anzahl Seiten = aufrunden(Gesamtzahl in der DB / Anzahl pro Seite)
    ...
    public static function paginate(Query $query, $pageSize = 10, $currentPage = 1)
    {
        ...
        $currentPage = (int) $currentPage;if ($pageSize < 1) {
            $pageSize = 10;
        }if ($currentPage < 1) {
            $currentPage = 1;
