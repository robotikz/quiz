...
class Ses {
	 ...
	 * @return string
	 */
	public static function getUpDirTmp($path = null, $typ='') {
		// the absolute directory where uploaded files should be saved
		return Ses::getUpDirImg ($path) . "/" . $typ;
	}
	
	public static function getUpDirImg($path = null) {
		if ($path!=null){
			return rtrim($path, '/');
		}
	...
	/*
	 * $length - the length of the returned random-number
	 */
	public static function rnr($length = 10) {
		$token = "";
		$codeAlphabet = "";
		// $codeAlphabet. = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
		$codeAlphabet .= "0123456789";
	...
	public static function after($thiz, $inthat) {
		if (! is_bool ( strpos ( $inthat, $thiz ) ))
			return substr ( $inthat, strpos ( $inthat, $thiz ) + strlen ( $thiz ) );
		return $inthat;
	}
	public static function after_last($thiz, $inthat) {
		if (! is_bool ( Ses::strrevpos ( $inthat, $thiz ) ))
			return substr ( $inthat, Ses::strrevpos ( $inthat, $thiz ) + strlen ( $thiz ) );
		return $inthat;
	}
	public static function before($thiz, $inthat) {
		if (! is_bool ( strpos ( $inthat, $thiz ) ))
			return substr ( $inthat, 0, strpos ( $inthat, $thiz ) );
		return $inthat;
	}
	public static function before_last($thiz, $inthat) {
		if (! is_bool ( Ses::strrevpos ( $inthat, $thiz ) ))
			return substr ( $inthat, 0, Ses::strrevpos ( $inthat, $thiz ) );
		return $inthat;
	}

	// TODO: check if it returns not double strings
	public static function between($thiz, $that, $inthat) {
		return before ( $that, after ( $thiz, $inthat ) );
	}
	public static function between_last($thiz, $that, $inthat) {
		return after_last ( $thiz, before_last ( $that, $inthat ) );
	}
	...
	public static function cast($destination, $sourceObject) {
		...
		foreach ( $sourceProperties as $sourceProperty ) {
			...
			$value = $sourceProperty->getVal ( $sourceObject );
	...
	/**
	 * Return array of random elements, return type is always array
	 *
	 * @param unknown $array
	 * @param number $amount
	 * @return array[]
